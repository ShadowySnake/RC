...
static int callback_print(void *str,int argc, char **argv,char **azColName)
{
  int i;
  char* data =(char*) str;
  for(i = 0; i<argc ; i++)
...
// cele doua callbackuri sunt inspirate de pe internet,
// linkul de unde m-am inspirat este in raport
  ...
  while(1){
    ...
    // perror("Eroare la read de la client.");
     ...
     if(strcmp(received,"No") == 0) write(th.cl,"Log in first!",sizeof("Log in first!"));
       ...
       rc = sqlite3_exec(database,sql,callback_print,str,&errmsg);
     ...
     if(strcmp(received,"No") == 0) write(th.cl,"Log in first!",sizeof("Log in first!"));
       ...
       strcpy(output1,str+11); //tine minte cati sunt inregistrati
       //printf("%s\n",output1);
       //printf("%ld\n",strlen(output1));
       output1[strlen(output1)-1]=0;
       //printf("%s\n",output1);
       ...
       rc = sqlite3_exec(database,sql,callback,str,&errmsg);
       strcpy(output2,str+13); // tine minte maxplayers
       output2[strlen(output2)-1]=0;
       //printf("%s\n",output2);

       sql[0] = 0;
       str[0] = 0;
       sprintf(sql,"SELECT email FROM users WHERE username='%s';",nume);
       rc = sqlite3_exec(database,sql,callback,str,&errmsg);
       strcpy(email,str+8);
       ...
       if( (strcmp(output2,output1) == 0) && (strcmp(output1,"0") !=0))
       {
         char cmd[10000];
         char body[]="Hello!\nYou tried to register to the championship: ";
         strcat(body,nume_champ);
         strcat(body,".");
         strcat(body,"\nThere are far too many people registered at this championship. Sorry, registration failed.");
         sprintf(cmd,"echo '%s' | mail -s 'Fail Register Championship' %s",body,email);
         system(cmd);
         // merge guaranteed pentru yahoo, pe gmail se pare ca e un pic
         // buggy si de multe ori esueaza
         write(th.cl,"Too many! An email has also been sent.",sizeof("Too many! An email has also been sent."));
       }
        ...
        char cmd[10000];
        char body[]="Hello!\nYou managed to register to the championship: ";
        strcat(body,nume_champ);
        strcat(body,".");
        strcat(body,"\nAn email will be sent at a later time, when the rounds will be decided.");
        sprintf(cmd,"echo '%s' | mail -s 'Registered Championship' %s",body,email);
        system(cmd);
        // yahoo merge, gmail are buguri si nu se trimite
        write(th.cl,"Success! An email has also been sent.",sizeof("Success! An email has also been sent."));
       }
    }
}
     ...
     if(strcmp(received,"No") == 0) write(th.cl,"Log in first!",sizeof("Log in first!"));
       ...
       sprintf(sql,"SELECT championship FROM rounds WHERE championship='%s';",nume_champ);
       rc = sqlite3_exec(database,sql,callback,str,&errmsg);
       if(strstr(str,nume_champ)){

        sql[0]=0;
        str[0]=0;
        sprintf(sql,"SELECT * FROM rounds WHERE player1='%s' OR player2='%s';",nume,nume);
        rc = sqlite3_exec(database,sql,callback,str,&errmsg);
      ...
      else write(th.cl,"Nu te regasesti in nicio runda din acel campionat!",sizeof("Nu te regasesti in nicio runda din acel campionat!"));
     }
     else write(th.cl,"That round does not exist!",sizeof("That round does not exist!"));
    }
  }
     ...
     else if(strcmp(received,"delete_champ") == 0)
     {
       write(th.cl,"delete_champ",sizeof("delete_champ"));
       ...
       if(strcmp(received,"No") == 0) write(th.cl,"Log in first!",sizeof("Log in first!"));
         ...
         sprintf(sql,"DELETE FROM championship WHERE name='%s';",nume_champ);
         rc = sqlite3_exec(database,sql,callback,str,&errmsg);

         sql[0]=0;
         str[0]=0;

         sprintf(sql,"DELETE FROM registered WHERE championship='%s';",nume_champ);
         rc = sqlite3_exec(database,sql,callback,str,&errmsg);

         sql[0]=0;
         str[0]=0;

         sprintf(sql,"DELETE FROM rounds WHERE championship='%s';",nume_champ);
         rc = sqlite3_exec(database,sql,callback,str,&errmsg);

         write(th.cl,"The cahmpionship has been deleted!",sizeof("The cahmpionship has been deleted!"));
       }
     }
